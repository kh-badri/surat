...
<div class="container mx-auto px-4 py-8 sm:px-6 lg:px-8">
    <div class="bg-white shadow-xl rounded-xl p-6 md:p-8 lg:p-10">
        ...
        <form action="<?= base_url('customer/update/' . $customer['id']) ?>" method="post" class="space-y-6">
            <?= csrf_field() ?>
            <input type="hidden" name="_method" value="PUT">

            <div class="mb-4">
                <label for="id_customer" class="block text-gray-700 text-sm font-bold mb-2">ID Customer:</label>
                <input type="text" name="id_customer" id="id_customer" value="<?= old('id_customer', $customer['id_customer']) ?>"
                    class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent
                                 <?= $validation->hasError('id_customer') ? 'border-red-500' : '' ?>" required>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="nama_customer" id="nama_customer" value="<?= old('nama_customer', $customer['nama_customer']) ?>"
                    class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent
                                 <?= $validation->hasError('nama_customer') ? 'border-red-500' : '' ?>" required>
            ...
            <div class="mb-4">
                ...
                <textarea name="alamat" id="alamat" rows="3"
                    class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent
                                 <?= $validation->hasError('alamat') ? 'border-red-500' : '' ?>"><?= old('alamat', $customer['alamat']) ?></textarea>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="no_hp" id="no_hp" value="<?= old('no_hp', $customer['no_hp']) ?>"
                    class="shadow appearance-none border rounded-lg w-full py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent

...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="bg-white p-6 sm:p-8 rounded-xl shadow-lg max-w-2xl mx-auto">
        ...
        <div class="pb-4 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-500 mt-1">Ubah data siswa yang sudah ada di dalam sistem.</p>
        </div>

        <form action="<?= site_url('siswa/' . $siswa['id_siswa']) ?>" method="post" class="mt-8 space-y-6">
            <?= csrf_field() ?>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <select name="tahun" id="tahun" class="pl-10 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out <?= ($validation->hasError('tahun')) ? 'border-red-500' : ''; ?>">
                        ...
                        ?>
                            <option value="<?= $tahun ?>" <?= old('tahun', $siswa['tahun']) == $tahun ? 'selected' : '' ?>><?= $tahun ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
                ...
                <div class="relative">
                    ...
                    <select name="kecamatan_id" id="kecamatan_id" class="pl-10 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out <?= ($validation->hasError('kecamatan_id')) ? 'border-red-500' : ''; ?>">
                        <?php foreach ($wilayah_list as $w) : ?>
                            <option value="<?= $w['id'] ?>" <?= ($siswa['kecamatan_id'] == $w['id']) ? 'selected' : '' ?>>
                                <?= esc($w['kecamatan']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div>
                ...
                <div class="relative">
                    ...
                    <input type="number" name="jumlah" id="jumlah" value="<?= esc($siswa['jumlah'], 'attr') ?>" placeholder="e.g., 150" class="pl-10 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 ease-in-out <?= ($validation->hasError('jumlah')) ? 'border-red-500' : ''; ?>">
                </div>
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                    <div>
                        ...
                        <input type="text" value="<?= esc($siswa['probabilitas'], 'attr') ?>" class="mt-1 block w-full bg-gray-100 border border-gray-300 rounded-lg shadow-sm py-2 px-3 cursor-not-allowed" readonly>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600">CDF</label>
                        <input type="text" value="<?= esc($siswa['cdf'], 'attr') ?>" class="mt-1 block w-full bg-gray-100 border border-gray-300 rounded-lg shadow-sm py-2 px-3 cursor-not-allowed" readonly>
                    </div>
                    <div>
                        <label class="block text-xs font-medium text-gray-600">Batas</label>
                        <input type="text" value="<?= esc($siswa['batas'], 'attr') ?>" class="mt-1 block w-full bg-gray-100 border border-gray-300 rounded-lg shadow-sm py-2 px-3 cursor-not-allowed" readonly>
                    </div>
                </div>
            </div>

            <div class="flex items-center justify-end space-x-4 pt-4 border-t border-gray-200 mt-6">
                <a href="<?= site_url('siswa') ?>" class="px-4 py-2 text-sm font-medium text-gray-600 rounded-lg hover:bg-gray-100">
                    Batal
                </a>

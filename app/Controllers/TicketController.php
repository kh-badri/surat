...
// Buat rules validasi yang sederhana
$rules = [
'nama_customer_ticket' => 'required|max_length[100]',
'alamat_customer_ticket' => 'required',
'no_hp_customer_ticket' => 'required|max_length[20]',
'keluhan' => 'required|max_length[200]',
'status' => 'required|in_list[open,progress,closed,diproses,selesai]',
...
'role_petugas_ticket' => 'required|max_length[50]',
];
...
// Tambahkan validasi customer_id hanya jika tidak kosong (mode pilih customer)
if (!empty($dataToUpdate['customer_id'])) {
if ($dataToUpdate['customer_id'] === $oldTicket['customer_id']) {
$rules['customer_id'] = 'required';
} else {
$rules['customer_id'] = 'required|is_not_unique[customer.id]';
}
}
...
'max_length' => 'Kode tiket maksimal 50 karakter.'
],
...
'required' => 'No HP customer harus diisi.',
'max_length' => 'No HP customer maksimal 20 karakter.'
],
'keluhan' => [
'required' => 'Keluhan harus diisi.',
'max_length' => 'Keluhan maksimal 200 karakter.'
],
'status' => [
...
'customer_id' => [
'is_not_unique' => 'Customer yang dipilih tidak valid.'
]
];

// Validasi data
if (!$this->validate($rules, $dataToUpdate, $messages)) {
// Debug: Log validation errors
log_message('error', 'Validation failed on update: ' . json_encode($this->validator->getErrors()));
